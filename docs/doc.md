...
* --no-string-comments (or -c for short): When we extract ROM data to asm-file, for every instruction which references a text string index, we make a comment at the end of the line with the string contents by default. This option disables that functionalty.
 ...
 * --no-smart-linker (-n for short): This option will disable smart linking, a patching mode which is supposed to make the most of available size. If this is not used, shop data will be put in the original ROM region, and the code data will be put in the second free region. Use this if you want to store this data separately for whatever reason.
...
This section contains a list of strings, each with an index. For some reason the game uses 1-based indexing for strings, so to stay consistent with the game we do so too.

```
[strings]
1: "Hello.<n>Could I help youwith anything.<p>What<n>would you like?"
2: "Thank you<n>for shopping."
3: "This is not<n>enough Golds."
```
This is the start of the strings section when the original game data has been extracted. Strings are enclosed by double quotes, and we use special syntax for some characters. Valid characters you can write directly are, based on my own investigation:
* A-Z, a-z, 0-9
* . ? ' , ! - _
...
Strings are densely packed in the original game code and we can't extend this section (without making game code modifications at least).

Strings indexes are given by 1 byte arguments in the code, and they are 1-indexed, meaning you can't use more than 254 distinct strings.

Many strings in the game seem to be missing spaces between words, but in those cases they are taking advantage of the fact that line breaks will occurr there.
...
|--------------|----------------|-----------|-------------|
| End          | —              | —         | Ends the script |
| MsgNoskip    | string index   | —         | Shows unskippable message |
| MsgPrompt    | string index   | —         | Shows prompt message. Ends script if cancelled, continues otherwise |
| Msg          | string index   | —         | Shows skippable message, script ends if skipped |
...
| IfMsgPrompt  | string index   | label     | Shows prompt and jumps if accepted |
...
It will be clearer once we look at a script and inspect it. Here is the script for the dialogue with the wise old man in Tower of Fortress who wants an elixir to open the fountain.

```
.entrypoint 38
.textbox GENERIC
    IfQuest QUEST_TOWER_SPRING iscript_038_00
    MsgPrompt 89 ; "You've done wellto get here.<n>This is the<n>spring of Trunk.<p>You need Elixir<n>to revive the<n>spring.<p>Will you give meyour medicine?"
    IfItem SPECIAL_ELIXIR iscript_038_01
    Msg 91 ; "Come back with<n>the medicine."
    End
iscript_038_01:
    MsgNoskip 90 ; "This spring willflow again.<n>There are<n>two more.<p>You'd better go<n>and find them."
    LoseItem SPECIAL_ELIXIR
    SetQuest QUEST_TOWER_SPRING
    End
iscript_038_00:
    Msg 92 ; "As soon as waterflows in the<n>fountain,<p>go to Mascon andfind the poison<n>the Dwarfs hide,<p>otherwise the<n>Elves will be<n>destroyed."
    End
```
...
which displays message with string index 89 (the actual string contents are helpfully put in the comments) and prompts the user. If the user cancels the script terminates, but if the user accepts, the following instruction is run:

```IfItem SPECIAL_ELIXIR iscript_038_01```

This says that if the player has the elixir, we jump to label iscript_038_01.

Otherwise execution continues with another Msg-opcode, displaying message string 91, which tells the player to come back with the elixir. Then the script ends.
...
The wise man says the spring will flow again (message string 90), then the player loses the elixir, and the quest flag for this spring is set. Then the script ends.
...
```
    ...
    End
```
So if we talk to him after already finishing the quest, we get a message from string 92, which tells us something, before the script ends.
...
```
.textbox GENERIC
    MsgNoskip 38 ; "I've been on<n>a long journey.<p>I came back ..."
    End
```
...
Then assemble your file back to ROM and confirm that you get the expected results. The example above makes the Pink Shirt Guy show up in a portrait, it makes the textbox skippable - but if you don't skip it you get 1000 golds, 50 health, and you get the dragon slayer.
...
Try to plan your scripts and allocate string indexes in advance, so you don't spend a lot of time going between your code, editing strings and looking up string indexes.
...
After that you need to make sure that we never hit another textbox while the code is executing, and that all possible branches execution flow can take ultimately end with the End-opcode. After assembly, the application will try to re-parse your code by entering at each entry point and simulate execution for all possible branches. If this fails, your ROM will not be patched and you need to fix your asm-file.
